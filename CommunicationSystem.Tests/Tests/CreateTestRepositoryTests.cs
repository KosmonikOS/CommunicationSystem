...
namespace CommunicationSystem.Tests.Tests
{
    public class CreateTestRepositoryTests
    {
        ...
        public async Task ItShould_Return_Options_For_Question()
        {
            ...
            var sut = new CreateTestRepository(context);
            var test = context.Tests.FirstOrDefault();
            var question = context.Questions.Where(q => q.TestId == test.Id).FirstOrDefault();
            var expected = context.Options.Where(o => o.QuestionId == question.Id).AsNoTracking().ToList();
            //Act
        ...
        public async Task ItShould_Return_User_Answers_Async()
        {
            ...
            CreateTestDataInitialzier.Initialize(context);
            var sut = new CreateTestRepository(context);
            var expected = context.Questions.AsNoTracking().Select(q => context.StudentAnswers.FirstOrDefault(a => a.UserId == 1 && a.QuestionId == q.Id).Answer ?? "").ToList();
            //Act
